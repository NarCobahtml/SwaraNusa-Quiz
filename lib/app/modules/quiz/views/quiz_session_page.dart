...
class QuizSessionPage extends StatefulWidget {
  ...
  const QuizSessionPage.tebakGambar({super.key})
    : config = QuizFallbackQuestions.tebakGambarLevel1;

  const QuizSessionPage.tebakSuara({super.key})
    : config = QuizFallbackQuestions.tebakSuaraLevel1;

  const QuizSessionPage.sejarah({super.key})
    : config = QuizFallbackQuestions.sejarahLevel1;
...
class _QuizSessionPageState extends State<QuizSessionPage> {
  ...
  void initState() {
    super.initState();
    _controller = QuizSessionController(config: widget.config)..load();
  }
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        if (_controller.phase == QuizSessionPhase.error) {
          return _StatusScaffold(
            title: widget.config.title,
            child: _ErrorState(
              message: _controller.errorMessage ?? 'Gagal memuat soal.',
              onRetry: () {
                _controller.load();
              },
            ),
        ...
        if (_controller.phase == QuizSessionPhase.loading || question == null) {
          return _StatusScaffold(
            title: widget.config.title,
        ...
        return QuizPageLayout(
          ...
          question: QuizQuestionMedia(
            key: ValueKey('${question.id}_${_controller.phase.name}'),
            question: question,
          ),
  ...
  Widget _buildBody(QuizSessionQuestion question) {
